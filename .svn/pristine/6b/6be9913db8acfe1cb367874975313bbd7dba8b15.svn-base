...
<body class="contPadding">
	...
	layui.use(['table','form'], function(){
	  ...
	  table.render({
	    elem: '#tableId'
	    ,url:'${ctx}/zsdindustry/toListData'
	    ,cellMinWidth: 80
	    ,page: true
	    ,cols: [[
	      {type:'checkbox'}
	    ...
	    /*  ,{field:'parentId', title: '父类型id'} */
	      ,{field:'createTime', title: '创建时间',templet: '#timeConversion'}
	...
	function doBatchDelete(title,array){
		layer.confirm(title, function(index) {
			$.ajax({
			     url: '${ctx}/zsdindustry/batchDelete',
			     type:"post",
			     //async:false,
			     dataType:"json",
			     data:{ids:array},
	...
	function doForm(title,id){
		layer.open({
			  ...
			  maxmin: true,
			  area: ['100%','100%'],
			  content: '${ctx}/zsdindustry/toForm?id='+id,
			  btn: ['立即提交'],
	...
	function doDetails(title,id){
		layer.open({
			  ...
			  maxmin: true,
			  area: ['100%','100%'],
			  content: '${ctx}/zsdindustry/toDetails?id='+id
		 });
	}
